        ...
        <div className="table-wrapper">
          <table className="table bookings-table">
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>Đang tải dữ liệu...</td>
                </tr>
              )}
              {!loading && error && (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', color: '#dc2626' }}>{error}</td>
                </tr>
              )}
              {!loading && !error && filteredBookings.length === 0 && (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>Không có booking phù hợp bộ lọc.</td>
                </tr>
              )}
              {!loading && !error && filteredBookings.map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="bookings-status-cell"><span className={`badge ${s.cls}`}>{s.label}</span></td>
                    <td className="bookings-action-cell">
                      ...
                      <button
                        className="btn btn-secondary btn-sm bookings-review-btn"
                        onClick={(e) => e.stopPropagation()}
                        title="Khung tính năng - sẽ triển khai sau"
                      >
                        Nhận xét/Đánh giá
                      </button>
                      </div>
